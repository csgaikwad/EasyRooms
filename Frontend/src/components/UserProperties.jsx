    ...
    <div className="block md:grid grid-cols-1  xl:grid-cols-2  gap-8  my-4 justify-items-center p-4 ">
      {userProperties.map((property) => (
        ...
        >
          ...
          <div
            className="hidden sm:flex w-full mt-2 items-center justify-center text-xl bg-red-500 opacity-50 hover:opacity-100 transition-transform duration-300 transform hover:scale-125 hover:z-10   cursor-pointer "
            onClick={() => {
              navigate("/property/" + property._id);
            }}
          >Edit
            <img
              className=" border-2 border-white m-4 size-10    rounded-full p-1"
              src="/pencil.svg"
              alt="edit"
            />
          </div>
